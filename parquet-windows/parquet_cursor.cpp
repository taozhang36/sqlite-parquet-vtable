...
int64_t int96toMsSinceEpoch(const parquet::Int96& rv) {
	const uint64_t ns_low = rv.value[0]; 
	const uint64_t ns_high = static_cast<uint64_t>(rv.value[1]) << 32; 
	const uint64_t ns = ns_low + ns_high;

	const uint64_t julianDay = rv.value[2];
	const uint64_t daysSinceEpoch = julianDay - 2440588ULL; 

	uint64_t days_sec_high, days_sec_low;
	days_sec_low = UMUL128(daysSinceEpoch, 86400ULL, &days_sec_high);

	uint64_t total_ns_high, total_ns_mid, total_ns_low;
	total_ns_low = UMUL128(days_sec_low, 1000000000ULL, &total_ns_mid);
	total_ns_high = days_sec_high * 1000000000ULL + total_ns_mid;

	uint64_t final_low = total_ns_low + ns;
	uint64_t carry = (final_low < total_ns_low) ? 1 : 0; 
	uint64_t final_high = total_ns_high + carry;

	const uint64_t divisor = 1000000ULL;

	uint64_t high_quotient = final_high / divisor;
	uint64_t high_remainder = final_high % divisor;

	uint64_t low_combined = (high_remainder << 64) | final_low;
	uint64_t low_quotient = low_combined / divisor;

	return static_cast<int64_t>((high_quotient << 64) + low_quotient);
}
